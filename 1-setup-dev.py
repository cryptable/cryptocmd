...
def install_catch2():
    if not os.path.isdir("Catch2"):
        ...
        subprocess.run('git branch -d latest-Catch2', shell=True, check=True)
        subprocess.run('git pull', shell=True, check=True)
        os.chdir("..")

    os.chdir("Catch2")
    subprocess.run('git fetch --all --tags', shell=True, check=True)
    subprocess.run('git checkout tags/v2.12.1 -b latest-Catch2', shell=True, check=True)
...
def install_openssl_win32():
    ...
    pwait = subprocess.run('git checkout OpenSSL_1_1_1-stable', shell=True, check=True)
