...
def install_cryptablepki():
    ...
    subprocess.run('conan install ..', shell=True)
    subprocess.run('cmake .. -DCMAKE_BUILD_TYPE=Release -DCONAN=1 -A x64', shell=True)
    ...
    # debug
    subprocess.run('cmake .. -DCMAKE_BUILD_TYPE=Debug -DCONAN=1 -A x64', shell=True)
